...
actor CocktailModelActor: Sendable {
    ...
    func fetchFavorites() async throws -> [FavoritesDTO] {
        print("[ModelActor] Fetching favorites...")
        let fetchDescriptor = FetchDescriptor<Favorites>(sortBy: [SortDescriptor(\Favorites.name)])
    ...
    func addFavorite(name: String) async throws {
        print("[ModelActor] Adding favorite: \(name)")
